...
/* Note: the HID_USAGE_SENSORS* usage codes that Zephyr >= 4.4 also defines in
 * <zephyr/usb/class/hid.h> are no longer defined here, to avoid macro
 * redefinition warnings. This header must stay devicetree-safe (it is
 * included from .dtsi files via dt-bindings/zmk/keys.h), so it must not
 * include C headers.
 */
